#ifndef GRADIDO_BLOCKCHAIN_CORE_BENCH_BITMAP_BODY_H
#define GRADIDO_BLOCKCHAIN_CORE_BENCH_BITMAP_BODY_H
...
/*
 * The transaction sets of the index, one body compiled once per bitmap backend.
 *
 * This file is included by bench_bitmap_arnm.c, bench_bitmap_croaring32.c and
 * bench_bitmap_croaring64.c, each of which defines which backend it wants and what the
 * descriptor it exposes is called. Everything here is static, so the three copies never meet.
 *
 ...
 *
 * CRoaring is built with ROARING_DISABLE_X64 throughout: the CPU this index is meant for has
 * no x64 SIMD, and the paths that would use it measured the same as the scalar ones on a
 * machine that has it. What every row answers is carried back as a checksum, and the driver
 * refuses to print a table whose backends disagree.
 */
...
#define BACKEND_NAME "CRoaring 4.6.1 roaring64_bitmap_t, scalar"
#define BACKEND_LABEL "cr64"
#else
#define BACKEND_NAME "CRoaring 4.6.1 roaring64_bitmap_t, x64 SIMD"
#define BACKEND_LABEL "cr64"
#endif
...
/** Nothing to give back: what the sets held went through the counted allocator. */
...
/** What the sets hold, and what was taken from the host to hold it. */
static void backend_memory_mib(double *live, double *reserved) {
  // requested bytes only: glibc adds at least 8 bytes of header and rounds to 16 per block
  const proto_counted_stats stats = proto_counted_get_stats();
  *live = (double)stats.current_bytes / 1048576.0;
  *reserved = (double)stats.peak_bytes / 1048576.0;
}
 ...
 */
#if defined(ROARING_DISABLE_X64)
#define BACKEND_NAME "CRoaring 4.6.1 roaring_bitmap_t (32 bit), scalar"
#define BACKEND_LABEL "cr32"
#else
#define BACKEND_NAME "CRoaring 4.6.1 roaring_bitmap_t (32 bit), x64 SIMD"
#define BACKEND_LABEL "cr32"
#endif
...
/** Gives the arenas back, so the next chain starts on an empty pool and measures itself. */
static void backend_release(void) {
  arnm_graded_block_pool_release(&g_index_pool, NULL);
...
/** What the sets hold, and what the arenas behind them reserved for it. */
static void backend_memory_mib(double *live, double *reserved) {
  arnm_multi_arena_stats chain = {0};
  (void)arnm_multi_arena_measure(g_index_pool.source, &chain);
  *live = (double)g_index_pool.lent_bytes / 1048576.0;
  *reserved = (double)chain.reserved / 1048576.0;
}
...
/** Runs one row and writes what it cost and what it answered. */
static void run_query(
    bench_bitmap_report *report, const char *name, query_fn fn, const query_context *ctx
) {
  checksum sum = {0, 0};
  ...
  const uint64_t queries = (uint64_t)ctx->repeat * ctx->key_count;
  if (report->row_count == BENCH_BITMAP_ROW_MAX) { return; }
  bench_bitmap_row *row = &report->rows[report->row_count++];
  row->name = name;
  row->nanos = queries ? (double)nanos / (double)queries : 0.0;
  row->counts = sum.counts;
  row->hash = sum.values;
}

static void body_run(const bench_chain_data *data, bench_bitmap_report *report) {
  ...
  prepared p;
  prepare(&p, data);
  report->addresses = p.unique_keys;
  ...
  arnm_mono_timer_reset(&timer);
  index_build(&index, &p);
  const uint64_t build_nanos = (uint64_t)arnm_mono_timer_nanos(timer);
  uint64_t adds = 0;
  for (uint32_t i = 0; i < index.address_count; ++i) {
  ...
  report->build_ms = (double)build_nanos / 1e6;
  report->adds = adds;
  report->nanos_per_add = adds ? (double)build_nanos / (double)adds : 0.0;
  backend_memory_mib(&report->live_mib, &report->reserved_mib);
  report->min_tx = index.min_tx;
  report->max_tx = index.max_tx;
  ...
  if (data->count) {
    ...
    const int64_t from = first + (last - first) / 4;
    const int64_t to = first + 3 * (last - first) / 4;
    uint32_t i = 0;
  ...
  report->range_min = ctx.range_min;
  report->range_max = ctx.range_max;

  const uint32_t sample_count = p.unique_keys < SAMPLE_KEYS ? p.unique_keys : SAMPLE_KEYS;
  uint32_t *sample =
  ...
  const uint32_t top_count = p.unique_keys < TOP_KEYS ? p.unique_keys : TOP_KEYS;
  report->busiest = top_count ? set_cardinality(&index.addresses[by_activity[0]].balance) : 0ull;

  ctx.keys = sample;
  ctx.key_count = sample_count;
  ctx.repeat = 1;
  run_query(report, "sample: involved, newest 20", q_involved_newest, &ctx);
  run_query(report, "sample: involved, count in date range", q_involved_count_range, &ctx);
  run_query(
      report, "sample: balance+TRANSFER+range, count+page 2", q_balance_transfer_range_page, &ctx
  );
  run_query(report, "sample: balance, count all", q_balance_count_all, &ctx);
  ctx.keys = by_activity;
  ctx.key_count = top_count;
  ctx.repeat = TOP_REPEAT;
  run_query(report, "top20: involved, newest 20", q_involved_newest, &ctx);
  run_query(report, "top20: involved, count in date range", q_involved_count_range, &ctx);
  run_query(
      report, "top20: balance+TRANSFER+range, count+page 2", q_balance_transfer_range_page, &ctx
  );
  ctx.key_count = 1;
  ctx.repeat = WIDE_REPEAT;
  run_query(report, "wide: TRANSFER in range, count+newest 20", q_transfer_range_newest, &ctx);
  run_query(report, "wide: CREATION in range minus foreign coin", q_creation_without_foreign, &ctx);
  run_query(
      report, "wide: TRANSFER AND DEFERRED in range, count", q_transfer_and_deferred_range, &ctx
  );
...
const bench_bitmap_backend BENCH_BITMAP_SYMBOL = {BACKEND_NAME, BACKEND_LABEL, body_run};

#endif // GRADIDO_BLOCKCHAIN_CORE_BENCH_BITMAP_BODY_H
