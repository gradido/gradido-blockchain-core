...
//! \param alloc is optional, can be null, but then grd_error_details_fill will use malloc for each string
...
//! use own allocator to allocate memory for error messages and copy them over with memcpy, only if not set to null
//! if allocator is full, will use default malloc
