...
formats everything: `src/`, `include/`, `tests/unit/src/*.cpp` and `benchmarks/src/`. The tree
is kept formatted, so a run is a no-op unless you left something behind — check with
