...
static arnm_result community_uuid_from_pbtools(
    uint8_t community_uuid[ARNM_UUID_BINARY_SIZE], const struct pbtools_bytes_t *bytes
) {
  if (!community_uuid || !bytes) { return ARNM_ERROR_NULL_POINTER; }
  if (ARNM_UUID_BINARY_SIZE == bytes->size) {
    memcpy(community_uuid, bytes->buf_p, ARNM_UUID_BINARY_SIZE);
  } else if (0 == bytes->size) {
    memset(community_uuid, 0, ARNM_UUID_BINARY_SIZE);
  } else {
...
static arnm_result memory_block_from_pbtools(
    arnm_memory_block *dst, const struct pbtools_bytes_t *bytes, arnm *allocator
) {
...
static arnm_result account_balance_from_pbtools(
    grdw_account_balance *account_balance,
    const struct proto_gradido_account_balance_t *pb_account_balance
) {
  if (!account_balance || !pb_account_balance) { return ARNM_ERROR_NULL_POINTER; }
  if (pb_account_balance->pubkey.size != SIGN_PUBLIC_KEY_SIZE) { return ARNM_ERROR_INVALID_PARAM; }
...
static arnm_result encrypted_memo_from_pbtools(
    grdw_encrypted_memo *encrypted_memo,
    const struct proto_gradido_encrypted_memo_t *pb_encrypted_memo,
    arnm *allocator
) {
  if (!encrypted_memo || !pb_encrypted_memo || !allocator) { return ARNM_ERROR_NULL_POINTER; }
...
static arnm_result signature_pair_from_pbtools(
    grdw_signature_pair *signature_pair,
    const struct proto_gradido_signature_pair_t *pb_signature_pair
) {
  if (!signature_pair || !pb_signature_pair) { return ARNM_ERROR_NULL_POINTER; }
...
static arnm_result timestamp_from_pbtools(
    grdd_timestamp *timestamp, const struct proto_gradido_timestamp_t *pb_timestamp
) {
  if (!timestamp || !pb_timestamp) { return ARNM_ERROR_NULL_POINTER; }
  timestamp->nanos = pb_timestamp->nanos;
  timestamp->seconds = pb_timestamp->seconds;
  return ARNM_SUCCESS;
}

static arnm_result timestamp_seconds_from_pbtools(
    grdw_timestamp_seconds *timestamp_seconds,
    const struct proto_gradido_timestamp_seconds_t *pb_timestamp_seconds
) {
  if (!timestamp_seconds || !pb_timestamp_seconds) { return ARNM_ERROR_NULL_POINTER; }
  timestamp_seconds->seconds = pb_timestamp_seconds->seconds;
  return ARNM_SUCCESS;
}

static arnm_result transfer_amount_from_pbtools(
    grdw_transfer_amount *transfer_amount,
    const struct proto_gradido_transfer_amount_t *pb_transfer_amount
) {
  if (!transfer_amount || !pb_transfer_amount) { return ARNM_ERROR_NULL_POINTER; }
  if (pb_transfer_amount->pubkey.size != SIGN_PUBLIC_KEY_SIZE) { return ARNM_ERROR_INVALID_PARAM; }
...
static arnm_result hiero_account_id_from_pbtools(
...
static arnm_result hiero_transaction_id_from_pbtools(
...
static arnm_result ledger_anchor_from_pbtools(
  ...
  if (!ledger_anchor || !pb_ledger_anchor) { return ARNM_ERROR_NULL_POINTER; }
...
static arnm_result community_friends_update_from_pbtools(
  ...
  if (!community_friends_update || !pb_community_friends_update) { return ARNM_ERROR_NULL_POINTER; }
  community_friends_update->color_fusion = pb_community_friends_update->color_fusion;
  return ARNM_SUCCESS;
}

static arnm_result community_root_from_pbtools(
    grdw_community_root *community_root,
    const struct proto_gradido_community_root_t *pb_community_root
) {
  if (!community_root || !pb_community_root) { return ARNM_ERROR_NULL_POINTER; }
...
static arnm_result gradido_creation_from_pbtools(
    grdw_gradido_creation *gradido_creation,
    const struct proto_gradido_gradido_creation_t *pb_gradido_creation
) {
  if (!gradido_creation || !pb_gradido_creation) { return ARNM_ERROR_NULL_POINTER; }
  arnm_result result = ARNM_ERROR_NOT_INITIALIZED;
  result =
...
static arnm_result gradido_transfer_from_pbtools(
    grdw_gradido_transfer *gradido_transfer,
    const struct proto_gradido_gradido_transfer_t *pb_gradido_transfer
) {
  if (!gradido_transfer || !pb_gradido_transfer) { return ARNM_ERROR_NULL_POINTER; }
...
static arnm_result gradido_deferred_transfer_from_pbtools(
...
static arnm_result gradido_redeem_deferred_transfer_from_pbtools(
...
static arnm_result gradido_timeout_deferred_transfer_from_pbtools(
...
static arnm_result register_address_from_pbtools(
    grdw_register_address *register_address,
    const struct proto_gradido_register_address_t *pb_register_address
) {
  if (!register_address || !pb_register_address) { return ARNM_ERROR_NULL_POINTER; }
...
arnm_result grdm_transaction_body_from_pbtools(
  ...
  if (pb_transaction_body->other_community_uuid.size) {
    result = arnm_alloc(&transaction_body->other_community_uuid, ARNM_UUID_BINARY_SIZE, allocator);
    if (ARNM_SUCCESS != result) { return result; }
...
arnm_result grdm_gradido_transaction_from_pb(
    grdw_gradido_transaction *tx,
    const struct proto_gradido_gradido_transaction_t *pbtx,
    arnm *allocator
) {
  if (!tx || !pbtx) { return ARNM_ERROR_NULL_POINTER; }
  arnm_result result = ARNM_ERROR_NOT_INITIALIZED;
...
arnm_result grdm_confirmed_transaction_from_pb(
  ...
  if (pb_confirmed_tx->running_hash.size != GENERIC_HASH_SIZE) { return ARNM_ERROR_INVALID_PARAM; }
