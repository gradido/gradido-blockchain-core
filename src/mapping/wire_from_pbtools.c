...
static hostmem_result account_balance_from_pbtools(
    grdw_account_balance *account_balance,
    const struct proto_gradido_account_balance_t *pb_account_balance
) {
  if (!account_balance || !pb_account_balance) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result signature_pair_from_pbtools(
    grdw_signature_pair *signature_pair,
    const struct proto_gradido_signature_pair_t *pb_signature_pair
) {
  if (!signature_pair || !pb_signature_pair) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result hiero_account_id_from_pbtools(
    grdw_hiero_account_id *hiero_account_id,
    const struct proto_gradido_account_id_t *pb_hiero_account_id
) {
  if (!hiero_account_id || !pb_hiero_account_id) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result ledger_anchor_from_pbtools(
  ...
  if (!ledger_anchor || !pb_ledger_anchor) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result community_friends_update_from_pbtools(
...
static hostmem_result community_root_from_pbtools(
    grdw_community_root *community_root,
    const struct proto_gradido_community_root_t *pb_community_root
) {
  if (!community_root || !pb_community_root) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result gradido_creation_from_pbtools(
    grdw_gradido_creation *gradido_creation,
    const struct proto_gradido_gradido_creation_t *pb_gradido_creation
) {
  if (!gradido_creation || !pb_gradido_creation) { return HOSTMEM_ERROR_NULL_POINTER; }
  hostmem_result result = HOSTMEM_ERROR_NOT_INITIALIZED;
  result =
...
static hostmem_result gradido_transfer_from_pbtools(
    grdw_gradido_transfer *gradido_transfer,
    const struct proto_gradido_gradido_transfer_t *pb_gradido_transfer
) {
  if (!gradido_transfer || !pb_gradido_transfer) { return HOSTMEM_ERROR_NULL_POINTER; }
...
static hostmem_result gradido_deferred_transfer_from_pbtools(
...
static hostmem_result gradido_redeem_deferred_transfer_from_pbtools(
...
static hostmem_result gradido_timeout_deferred_transfer_from_pbtools(
...
static hostmem_result register_address_from_pbtools(
    grdw_register_address *register_address,
    const struct proto_gradido_register_address_t *pb_register_address
) {
  if (!register_address || !pb_register_address) { return HOSTMEM_ERROR_NULL_POINTER; }
...
hostmem_result grdm_transaction_body_from_pbtools(
  ...
  if (pb_transaction_body->memos.length > 0) {
    ...
    if (memos_count >= 255) { return HOSTMEM_ERROR_ARRAY_INDEX_OUT_OF_BOUNDS; }
...
hostmem_result grdm_gradido_transaction_from_pb(
  ...
  if (pbtx->sig_map_p && pbtx->sig_map_p->sig_pair.length > 0) {
    ...
    if (sig_map_count >= 255) { return HOSTMEM_ERROR_ARRAY_INDEX_OUT_OF_BOUNDS; }
...
hostmem_result grdm_confirmed_transaction_from_pb(
  ...
  if (pb_confirmed_tx->running_hash.size != GENERIC_HASH_SIZE) {
    return HOSTMEM_ERROR_INVALID_PARAM;
  }
  ...
  if (pb_confirmed_tx->account_balances.length > 0) {
    ...
    if (account_balances_count >= 255) { return HOSTMEM_ERROR_ARRAY_INDEX_OUT_OF_BOUNDS; }
