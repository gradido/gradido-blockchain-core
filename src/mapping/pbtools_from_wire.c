...
static void community_uuid_from_wire(
    struct pbtools_bytes_t *dst, const uint8_t community_uuid[ARNM_UUID_BINARY_SIZE]
) {
  if (!community_uuid || !dst) { return; }

  dst->buf_p = (uint8_t *)community_uuid;
  dst->size = ARNM_UUID_BINARY_SIZE;
}
/*
 * simply copy ptr to dst, or do nothing if at least one parameter is NULL
 */
static void memory_block_from_wire(struct pbtools_bytes_t *dst, const arnm_memory_block *bytes) {
...
static arnm_result hiero_transaction_id_from_wire(
...
static arnm_result ledger_anchor_from_wire(
...
static arnm_result community_friends_update_from_wire(
...
static arnm_result community_root_from_wire(
...
static arnm_result gradido_creation_from_wire(
...
static arnm_result gradido_transfer_from_wire(
...
static arnm_result gradido_deferred_transfer_from_wire(
...
static arnm_result gradido_redeem_deferred_transfer_from_wire(
...
static arnm_result gradido_timeout_deferred_transfer_from_wire(
...
static arnm_result register_address_from_wire(
...
arnm_result grdm_transaction_body_from_wire(
  ...
  if (!transaction_body || !pb_transaction_body) { return ARNM_ERROR_NULL_POINTER; }
...
arnm_result grdm_gradido_transaction_from_wire(
  ...
  if (tx->sig_map_count > 0) {
    ...
    if (proto_gradido_gradido_transaction_sig_map_alloc(pbtx)) { return ARNM_ERROR_OUT_OF_MEMORY; }
...
arnm_result grdm_confirmed_transaction_from_wire(
  ...
  if (!confirmed_tx || !pb_confirmed_tx) { return ARNM_ERROR_NULL_POINTER; }
