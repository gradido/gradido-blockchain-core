...
static grdi_validate_result_type validateCommon(
  ...
  if (input_tx->confirmed_at.seconds < 946681200) {
    grd_error_details_fill_actual_is_number(
      error_details,
      "confirmedAt.seconds is to small",
      input_tx->confirmed_at.seconds,
      "> 946681200 (01.01.2000)"
    );
    return GRDI_VALIDATE_INVALID_FIELD;
  }
  ...
  if (abs(diff.seconds) > MAGIC_NUMBER_MAX_TIMESPAN_BETWEEN_CREATING_AND_RECEIVING_TRANSACTION_SECONDS) {
    grd_error_details_fill_actual_is_number(
      error_details,
      "timespan between created at and confirmed at are more than expected",
      abs(diff.seconds),
      "120 seconds"
    );
  ...
  if (input_tx->cross_group_type != GRDT_CROSS_GROUP_LOCAL)
  {
      ...
      GRDT_TRANSACTION_TRANSFER != input_tx->transaction_type)
    {
      grd_error_details_fill(
        error_details,
        "cross group transactions (currently) only possible with Transfer, Redeem Deferred Transfer and Community Friends Update",
        NULL,
        NULL
      );
    ...
    if (!input_tx->tx_pairing_community_uuid) {
      grd_error_details_fill(
        error_details,
        "missing pairing community id index for cross group transaction",
        NULL,
        NULL
      );
      return GRDI_VALIDATE_INVALID_FIELD;
    }
    if (GRDT_CROSS_GROUP_INBOUND == input_tx->cross_group_type && !input_tx->pairing_ledger_anchor) {
      grd_error_details_fill(
        error_details,
        "empty pairing ledger anchor for Inbound Cross Group Transaction",
        NULL,
        NULL
      );
      return GRDI_VALIDATE_INVALID_FIELD;
  ...
  if (GRDT_BALANCE_DERIVATION_UNSPECIFIED == input_tx->balance_derivation_type) {
    grd_error_details_fill(
      error_details,
      "balanceDerivationType is unspecified",
      NULL,
      NULL
    );
    return GRDI_VALIDATE_INVALID_FIELD;
  ...
  if (input_tx->signature_pairs_count) {
    for (int i = 0; i < input_tx->signature_pairs_count; i++) {
      ...
      if (IS_EMPTY_PUBLIC_KEY(sigPair->public_key)) {
        grd_error_details_fill(
          error_details,
          "empty public key in signature map",
          NULL,
          NULL
        );
        return GRDI_VALIDATE_INVALID_FIELD;
      ...
      if (IS_EMPTY_SIGNATURE(sigPair->signature)) {
        grd_error_details_fill(
          error_details,
          "empty signature in signature map",
          NULL,
          NULL
        );
        return GRDI_VALIDATE_INVALID_FIELD;
      ...
      if (options->enable_verify) {
        ...
        if (crypto_sign_verify_detached(
          sigPair->signature,
          input_tx->body_bytes.data,
          input_tx->body_bytes.size,
          sigPair->public_key
        ) != 0) {
          grd_error_details_fill(
            error_details,
            "on of the signatures cannot be verified",
            NULL,
            NULL
          );
...
grdi_validate_result_type grdi_validate_complete_transaction(
  ...
  return validateCommon(input_tx, options, error_details);
}
