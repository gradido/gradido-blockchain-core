...
arnm_result grdc_sign_key_pair_generate_from_seed(
  ...
  if (!sign_key_pair || !seed) { return ARNM_ERROR_NULL_POINTER; }
...
arnm_result grdc_sign_key_pair_copy_slip10_public_key(
  ...
  if (!slip10_public_key || !sign_key_pair) { return ARNM_ERROR_NULL_POINTER; }
...
arnm_result grdc_sign_key_pair_derive(
  ...
  if (!sign_key_pair || !sign_parent_key_pair) { return ARNM_ERROR_NULL_POINTER; }
  if (index >= HARDENED_KEY_BITMASK) { return ARNM_ERROR_INVALID_PARAM; }
...
arnm_result grdc_sign_key_pair_derive_uuid(
  ...
  if (!sign_key_pair || !sign_parent_key || !user_uuid) { return ARNM_ERROR_NULL_POINTER; }
...
arnm_result grdc_sign_key_pair_derive_account_from_community(
  ...
  if (!sign_key_pair || !community_root_seed || !user_uuid) { return ARNM_ERROR_NULL_POINTER; }
  if (!account_index) { return ARNM_ERROR_INVALID_PARAM; }
  // community root key
  arnm_result result =
