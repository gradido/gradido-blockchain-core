...
hostmem_result grdc_sign_key_pair_copy_slip10_public_key(
  ...
  if (!slip10_public_key || !sign_key_pair) { return HOSTMEM_ERROR_NULL_POINTER; }
...
hostmem_result grdc_sign_key_pair_derive(
  ...
  if (!sign_key_pair || !sign_parent_key_pair) { return HOSTMEM_ERROR_NULL_POINTER; }
  if (index >= HARDENED_KEY_BITMASK) { return HOSTMEM_ERROR_INVALID_PARAM; }
...
hostmem_result grdc_sign_key_pair_derive_account_from_community(
  ...
  if (!account_index) { return HOSTMEM_ERROR_INVALID_PARAM; }
  // community root key
