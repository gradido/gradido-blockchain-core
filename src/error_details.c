...
{
  return error_details && !error_details->message && !error_details->actual && !error_details->expected && !error_details->used_default_malloc_flag;
}
...
grd_result grd_error_details_fill(grd_error_details* error_details, const char* message, const char* actual, const char* expected)
{
  if (!error_details) { return GRD_ERROR_NULL_POINTER; }
  ...
  if (message) {
    error_details->used_default_malloc_flag |= alloc_and_fill_field(&error_details->message, message, error_details->allocator, 1);
  }
  if (actual) {
    error_details->used_default_malloc_flag |= alloc_and_fill_field(&error_details->actual, actual, error_details->allocator, 2);
  }
  if (expected) {
    error_details->used_default_malloc_flag |= alloc_and_fill_field(&error_details->expected, expected, error_details->allocator, 4);
  }
  return GRD_SUCCESS;
}
...
grd_result grd_error_details_fill_actual_is_number(
  ...
  char strBuffer[22]; memset(strBuffer, 0, 22);
