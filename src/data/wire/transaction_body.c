...
hostmem_result grdw_transaction_body_decode(
  ...
  if (!binarySrc->size || !allocator->capacity) { return HOSTMEM_ERROR_INVALID_PARAM; }
  ...
  hostmem_memory_block_realloc(&pbBuffer, (uint32_t)proto_body->base.heap_p->pos, allocator);
  if (PBTOOLS_OUT_OF_MEMORY == -resultSize) { return HOSTMEM_ERROR_OUT_OF_MEMORY; }
  if (resultSize < 0 || (uint32_t)resultSize != binarySrc->size) {
