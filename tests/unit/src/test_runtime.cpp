...
static const uint8_t community_uuid[16] = {
    0x01, 0x9e, 0x2c, 0x31, 0xa3, 0x03, 0x75, 0xc0,
...
TEST(RuntimeTest, ConfirmedTransaction_Decode_ToRuntime_CommunityRoot) {
  ...
  ASSERT_EQ(grdw_transaction_body_decode(&body, &confirmed_tx.transaction.body_bytes, &mem), GRD_SUCCESS);

  grdr_complete_transaction tx;
  ASSERT_EQ(grdm_complete_transaction_from_wire(&tx, &body, &confirmed_tx, community_uuid), GRD_SUCCESS);
}
