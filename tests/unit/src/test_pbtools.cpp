...
static std::string toBase64(arnm_memory_block *data, int variant = sodium_base64_VARIANT_ORIGINAL) {
...
static std::string toHex(arnm_memory_block *data) {
  if (!data || !data->size) { return ""; }
...
static arnm_memory_block fromHex(const char *hex) {
  arnm_memory_block result{};
...
TEST(PBToolsTest, TransactionBody_Decode) {
  ...
  ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
...
TEST(PBToolsTest, DecodeOnAnExhaustedArenaSaysOutOfMemory) {
  ...
  arnm_memory_block bin{garbage, sizeof(garbage)};

  arnm mem{};
  ASSERT_EQ(arnm_init_arena(&mem, 1024), ARNM_SUCCESS);
  uint8_t *hog = nullptr;
  ASSERT_EQ(arnm_alloc(&hog, 1024, &mem), ARNM_SUCCESS);
  ASSERT_EQ(arnm_arena_remaining(&mem), 0u)
      << "the arena has to be full for this test to mean anything";

  grdw_transaction_body body{};
  EXPECT_EQ(grdw_transaction_body_decode(&body, &bin, &mem), ARNM_ERROR_OUT_OF_MEMORY);
  grdw_gradido_transaction tx{};
  EXPECT_EQ(grdw_gradido_transaction_decode(&tx, &bin, &mem), ARNM_ERROR_OUT_OF_MEMORY);
  grdw_confirmed_transaction confirmed{};
  EXPECT_EQ(grdw_confirmed_transaction_decode(&confirmed, &bin, &mem), ARNM_ERROR_OUT_OF_MEMORY);
...
TEST(PBToolsTest, FailedDecodeKeepsItsWorkspaceForInspection) {
  ...
  arnm mem{};
  ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
  const uint32_t empty_remaining = arnm_arena_remaining(&mem);
  ASSERT_GT(empty_remaining, 0u);

  grdw_transaction_body body{};
  EXPECT_EQ(grdw_transaction_body_decode(&body, &bin, &mem), ARNM_ERROR_DECODE_FAILED);
  EXPECT_LT(arnm_arena_remaining(&mem), empty_remaining)
      << "the workspace is kept on purpose, do not release it";

  // and the caller clears it, as the caller always does
  arnm_reset(&mem);
  EXPECT_EQ(arnm_arena_remaining(&mem), empty_remaining);
  arnm_release(&mem);
}
...
TEST(PBToolsTest, WireEntryPointsSurviveANullPointer) {
  ...
  EXPECT_EQ(grdw_transaction_body_reserve_memos(nullptr, 4, &mem), ARNM_ERROR_NULL_POINTER);
  // the null check comes first: an out of range count must not decide the outcome for a body
  // that was never there
  EXPECT_EQ(grdw_transaction_body_reserve_memos(nullptr, 999, &mem), ARNM_ERROR_NULL_POINTER);

  // and a valid body still reserves as before
  grdw_transaction_body body{};
  grdw_transaction_body_init(&body);
  EXPECT_EQ(grdw_transaction_body_reserve_memos(&body, 4, &mem), ARNM_SUCCESS);
  EXPECT_EQ(body.memos_count, 4u);
  EXPECT_EQ(grdw_transaction_body_reserve_memos(&body, 256, &mem), ARNM_ERROR_INVALID_PARAM);
...
// ARNM_ERROR_ENCODE_FAILED for a malformed buffer, which sends a reader looking at the
...
TEST(PBToolsTest, DecodeOfGarbageReportsDecodeFailed) {
  ...
  }
  {
    ...
    grdw_gradido_transaction tx{};
    ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
    EXPECT_EQ(grdw_gradido_transaction_decode(&tx, &bin, &mem), ARNM_ERROR_DECODE_FAILED);
    arnm_release(&mem);
  }
  {
    arnm mem{};
    grdw_confirmed_transaction confirmed{};
    ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
    EXPECT_EQ(grdw_confirmed_transaction_decode(&confirmed, &bin, &mem), ARNM_ERROR_DECODE_FAILED);
    arnm_release(&mem);
  }
}
...
TEST(PBtoolsTest, TransactionBody_Encode_OtherCommunity) {
  ...
  arnm_memory_block communityUuid = fromHex(communityUuidHex);
  arnm_alloc(&body.other_community_uuid, 16, &mem);
  memcpy(body.other_community_uuid, communityUuid.data, 16);
  free(communityUuid.data);

  body.transaction_type = GRDT_TRANSACTION_NONE;
  uint8_t buffer[256]{};
  arnm_memory_block bufferPtr = {.data = buffer, .size = 256};
  int finalSize = 0;
  ASSERT_EQ(grdw_transaction_body_encode(&bufferPtr, &finalSize, &body, &mem), ARNM_SUCCESS);
...
TEST(PBtoolsTest, TransactionBody_Decode_OtherCommunity) {
  ...
  ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
  ASSERT_EQ(grdw_transaction_body_decode(&body, &bin, &mem), ARNM_SUCCESS);
  free(bin.data);
...
TEST(PBToolsTest, TransactionBody_Creation_Encode) {
  ...
  arnm_memory_block_alloc(&memo.memo, 13, &mem);
...
TEST(PBToolsTest, TransactionBody_Creation_Decode) {
  ...
  EXPECT_EQ(arnm_init_arena(&mem, BUFFER_SIZE), ARNM_SUCCESS);
  ASSERT_EQ(grdw_transaction_body_decode(&body, &bin, &mem), ARNM_SUCCESS);
  free(bin.data);
  EXPECT_EQ(body.created_at.seconds, createdAt1.seconds);
  EXPECT_EQ(body.created_at.nanos, createdAt1.nanos);
...
TEST(PBToolsTest, GradidoTransaction_Encode_CommunityRoot) {
  ...
  ASSERT_EQ(grdw_gradido_transaction_copy_sig_map(&tx, &signature, 0), ARNM_SUCCESS);
  uint8_t buffer[256]{};
  arnm_memory_block bufferPtr = {.data = buffer, .size = 256};
  int finalSize = 0;
  ASSERT_EQ(grdw_gradido_transaction_encode(&bufferPtr, &finalSize, &tx, &mem), ARNM_SUCCESS);
  bufferPtr.size = finalSize;
  auto base64 = toBase64(&bufferPtr);
...
TEST(PBToolsTest, GradidoTransaction_Decode_CommunityRoot_1000X) {
  ...
  ASSERT_EQ(arnm_init_arena_borrow(&mem, staticBuffer, BUFFER_SIZE * 2), ARNM_SUCCESS);
  char buffer[256];
  arnm_mono_timer_reset(&timeUsed);
  int i = 0;
  for (; i < 1000; ++i) {
    ...
    ASSERT_EQ(grdw_gradido_transaction_decode(&tx, &bin, &mem), ARNM_SUCCESS);
    ASSERT_EQ(grdw_transaction_body_decode(&body, &tx.body_bytes, &mem), ARNM_SUCCESS);
  ...
  arnm_mono_timer_string(buffer, 256, timeUsed);
...
TEST(PBToolsTest, GradidoTransaction_Encode_CommunityRoot_1000X) {
  ...
  ASSERT_EQ(arnm_init_arena_borrow(&mem, staticBuffer, 128), ARNM_SUCCESS);
  grdw_gradido_transaction_init(&tx);
  ...
  int finalSize = 0;
  arnm mem2{};
  ASSERT_EQ(arnm_init_arena_borrow(&mem2, staticBuffer2, 304), ARNM_SUCCESS);
  int i = 0;
  arnm_mono_timer_reset(&timeUsed);
  for (; i < 1000; ++i) {
    arnm_reset(&mem2);
    arnm_memory_block_alloc(&tx.body_bytes, 128, &mem2);
    ASSERT_EQ(grdw_transaction_body_encode(&tx.body_bytes, &finalSize, &body, &mem2), ARNM_SUCCESS);
    arnm_memory_block_realloc(&tx.body_bytes, finalSize, &mem2);
    ASSERT_EQ(grdw_gradido_transaction_encode(&bufferPtr, &finalSize, &tx, &mem2), ARNM_SUCCESS);
    arnm_memory_block_free(&tx.body_bytes, &mem2);
  }
  char timerBuffer[256];
  arnm_mono_timer_string(timerBuffer, 256, timeUsed);
...
TEST(PBToolsTest, GradidoTransaction_Decode_TimeoutDeferredTransfer) {
  ...
  ASSERT_EQ(grdw_gradido_transaction_decode(&tx, &serializedTx, &mem), ARNM_SUCCESS);
...
TEST(PBToolsTest, ConfirmedTransaction_Encode_CommunityRoot) {
  arnm mem{};
  ASSERT_EQ(arnm_init_arena(&mem, BUFFER_SIZE * 2), ARNM_SUCCESS);
  ...
  tx.transaction.body_bytes =
      fromBase64(communityRootTransactionBodyBase64, strlen(communityRootTransactionBodyBase64));
  ASSERT_EQ(grdw_gradido_transaction_reserve_sig_map(&tx.transaction, 1, &mem), ARNM_SUCCESS);
  grdw_signature_pair signature{};
  memcpy(signature.public_key, g_KeyPairs[0].public_key, 32);
  unsigned long long sig_len = 64;
  crypto_sign_detached(
  ...
  ASSERT_EQ(grdw_gradido_transaction_copy_sig_map(&tx.transaction, &signature, 0), ARNM_SUCCESS);
  uint8_t buffer[512]{};
  arnm_memory_block bufferPtr = {.data = buffer, .size = 512};
  int finalSize = 0;
  ...
  ASSERT_EQ(grdw_confirmed_transaction_reserve_account_balances(&tx, 1, &mem), ARNM_SUCCESS);
...
TEST(PBToolsTest, ConfirmedTransaction_Decode_CommunityRoot) {
  ...
  ASSERT_EQ(grdw_confirmed_transaction_decode(&tx, &base64, &mem), ARNM_SUCCESS);
  EXPECT_EQ(tx.id, 121);
